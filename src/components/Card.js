...
const StyledButton = styled.div`
  float: right;
  color: #dc7777;
  margin-right: ${props => `${props.col === 1 ? '-120px' : '-50px' }`};
  font-size: 20px;
`
...
class Card extends Component {
  ...
  render() {
    const { data, col, clicked } = this.props;
    ...
    const weekness = this.calculateWeakness(
      (iWeaknesses && iWeaknesses.length) || 0
    );
        ...
        <CardWrapper className={`col-${col}`}>
          <CardContainer>
            ...
            { clicked ? (
              ...
              <StyledButton className="action" onClick={this.props.onAdd}>
              Add
              </StyledButton>
            )}
